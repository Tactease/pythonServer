...
def add_multiple_missions_with_soldiers(schedule_json_str, new_missions, soldiers_json):
    try:
        ...
        for new_mission in new_missions:
            available_soldiers = find_available_soldiers(
                temporary_schedule, new_mission, soldiers
            )
            needed_soldiers_count = new_mission.soldierCount
            available_soldiers_list = list(available_soldiers)

            if (
                needed_soldiers_count is not None
                and len(available_soldiers_list) >= needed_soldiers_count
            ):
                ...
                return json.dumps(
                    {"error": "Not enough available soldiers for the mission"}
                )
